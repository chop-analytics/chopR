---
title: "CHOP Data Analytics R Standards"
last author: "Trailblazers Club, Office of Clinical Quality Improvement" 
last update date: August 2nd, 2018
output:
  md_document:
...
![](Source .rmd files/media/r_project_setup_1.png?raw=true)

As you can see, there is now an R project file saved within the directory (last file listed above). If you wanted to read in a csv from patient_lists, your read.csv statement in your script would look like this: 
...
To create a section in your .R file, add an short informative header as a comment, followed by at least 4 dashed lines: `----` . This will create a little arrow on the left hand side allowing you to collapse your code. You can also use the following short-cut to create sections: `Ctrl+Shift+R`
 ...
 ![](Source .rmd files/media/code_structure_1.png)
 ![](Source .rmd files/media/code_structure_2.png)
...
Tip: Placing several `#` above your section makes things look more readable when all your sections are collapsed:
 ...
 ![](Source .rmd files/media/code_structure_3.png)
...
![](Source .rmd files/media/code_structure_4.png)
...
![](Source .rmd files/media/code_ord_1.png)
 
_Code Chunk #2:_
The second code chunk is where you will be bringing your data into your R environment. This can be performed via a SQL statement, read.csv statement, etc. 

_Code Chunk #3:_ 
The third code chunk is where you should format all of your data that was pulled in code chunk #2 (i.e. case changes from UPPER to lower, date formatting, etc.)

_Subsequent Code Chunks:_ 	
...
----------|-------------------------------------------|----------|
tidyverse	|Contains a number of different packages that are useful for data manipulation (dplyr, lubridate, ggplot2, tidyr, etc.)	|https://www.r-pkg.org/pkg/tidyverse
devtools	|Allows installment of packages from github	|https://www.r-pkg.org/pkg/devtools
rmarkdown	|Create Rmarkdowns	|https://www.r-pkg.org/pkg/rmarkdown
odbc |	Connects to the CDW	| https://www.r-pkg.org/pkg/odbc
REDCapR	| Interacts with REDCap	| https://www.r-pkg.org/pkg/REDCapR
sendmailR	| Sends emails from R	| https://www.r-pkg.org/pkg/sendmailR
EasyHTMLReport	| Sends HTML reports to clients	| https://www.r-pkg.org/pkg/EasyHTMLReport
htmlTable | 	Creates HTML tables	| https://www.r-pkg.org/pkg/htmlTable
readxl	| Reads excel files	| https://www.r-pkg.org/pkg/readxl
writexl	| Writes excel files	| https://www.r-pkg.org/pkg/writexl
rocqi	| Easily creates spc charts and performs other common functions specific for analysts	| https://github.research.chop.edu/pages/CQI/pkg-spc/rocqi/docs/reference/
lintr |Reformats code to adhere to CHOP standards	|https://cran.r-project.org/web/packages/lintr/index.html 
data.table |	Reformats data frames easily for publishing	|https://cran.r-project.org/web/packages/data.table/data.table.pdf
highcharter|	Creates interactive graphs for data visualization deliverables|	https://cran.r-project.org/web/packages/highcharter/highcharter.pdf 
...
_Functions:_ All functions outside of dplyr, gpplot2, tidyr, knitr, odbc, and readr will be called using the format “package_name::function_name”.  This will make it easier for others to know what packages you’re using when employing certain functions.  
Example: `reshape2::melt()`

_Objects_: all objects will be names using lowercase letters and underscores (` _ `) rather than dots (`.`) or hyphens (`-`) to separate words in object names.  Object/column names should be succinct and informative.

Example: `meds_raw`, `vanc_ind`

_Argument:_ If a function call is too long to fit on a single line, use one line each for the function name, each argument, and the closing “)”. Each new line should be indented by two spaces.
Example:
...
_Operators (+, =, -, etc.):_ all operators should be preceded with and followed by a space. Exceptions to the this rule include the following operators: `^`, `::`,`:`,`!`

_Pipes:_ each function you pipe (`%>%`) should be on its own line (the pipe should end the line so that your code can run automatically).  Additionally, each new line (new function being called by a pipe in this case) should be indented by two spaces. R generally does this automatically for you.
